...
| --- | --- | --- | --- |
| `stromlinien-epoche1.html` | `drafts/` | active, partly ported | Epoche-I-Kernloop (Tag/Nacht, Plättchen, Anker-Ereignisse, Fundstellen, Zeremonie). Hex-Version (v2). Kernloop nach `app/src/stromlinien/` portiert. |
...
| `start-screen-v2.html` | `drafts/` | active | Startbildschirm v2: endlose Seed-Welt unter **Nebel des Ungespielten** — Gelände unsichtbar, bis ein Gebiet gespielt wurde (localStorage pro Seed). Startgebiet = Sechseck (Radius 3, 37 Felder), **frei formbar** per Tippen (muss zusammenhängen, 12–55 Felder). **Zeichen ◈**: deterministische Hinweise nahe markantem Gelände; ungespielt anonym, im gewählten Gebiet liefert jedes eine vage Aussage über das Gesamtgebiet (kein Ort, keine Art); nach dem Spielen zeigen sie ihre wahre Glyphe. **Epochen-Ausblick**: alle 5 Epochen mit Erwartung (Punkte + Stufen) fürs gewählte Gebiet. Start nur mit ≥1 Flussfeld („Wasser zu hören"). Debug: `?seed=…`, `?autostart`, `?epochs`. |
