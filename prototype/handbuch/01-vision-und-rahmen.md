...
**Höhendaten sind angebunden** (Prototyp): Sonnys LiDAR-DTM der Schweiz (50&nbsp;m, CC&nbsp;BY&nbsp;4.0) wird normalisiert (200-m-Raster, Region Alpenrhein) und der Bake klassifiziert daraus eine **Ebene&nbsp;2 (~400&nbsp;m/Hex)**: Steigung + Relief über dem lokalen Talboden → flach / **Hang** / Berg, dazu Höhenschummerung. Das echte Relief zeichnet Talboden, Hangbänder und Seitentäler (Schanfigg, Domleschg) ohne Handarbeit. Ein **Nebenstrang** nutzt dieselben DTMs für ein 1-km-Hexraster mit Mittel-, Min- und Maxhöhe je Feld (`pipeline/fetch/bake-eiszeit.mjs` → `eiszeit-labor-v3`) – zuerst über Linth- und Alpenrhein-Einzugsgebiet, seit v3 über den ganzen Alpenbogen; ausserhalb der Landesgrenze wurden Höhen geschätzt – **war** so bis Aug 2026: **die Lücke ist geschlossen.** Die Pipeline liest Höhen nicht mehr aus *einer* Datei, sondern aus einem Register mit Vorrang (`pipeline/sources/hoehen.manifest.json`) – mehrere DTMs, die erste Quelle mit einem Wert gewinnt (LiDAR vor Radar), blockweise gelesen und beim Lesen auf die Zielauflösung ausgedünnt, damit eine 10-m-Quelle nicht ein Gigabyte in den Speicher holt. Mit den echten DTMs für AT, DE und IT (alle Sonny, 50 m) dazu – Liechtenstein brauchte keine eigene Datei, es liegt bereits im Schweizer Raster – sind von 28 576 Feldern nur noch **10 ohne Deckung** (vorher 6 220, 21.8 %). Je Feld steht die Herkunft im Datenfile (`quelle` + `hoehenquellen`), und `--pruefen` sagt vor jedem Bake, was gedeckt ist. Was das freilegte, steht unter „Das Erbe der Eiszeit": die Kalibrierung war an die geschätzte Füllung gebunden, nicht ans echte Relief. **Seit v3 ist der Nebenstrang der ganze Alpenbogen** (4.7–14.9&nbsp;°O / 44.9–48.6&nbsp;°N, 744×475 = 353&nbsp;400 Felder aus fünf DTMs – FR steht dafür im Register jetzt auch auf `fuer: eiszeit`; NL fehlt weiter, aber nur für den Rhein). Zwei Dinge hat die Pipeline dabei gelernt, die über die Eiszeit hinausgehen: **ein Loch wird nicht mehr blind gefüllt** – geschätzt wird nur bis 12 Felder weit, alles weiter Entfernte trägt Bit&nbsp;3 *leer* (Höhe 0, ausserhalb der Modellfläche; Slowenien und Nordadria, 20&nbsp;375 Felder), weil eine nächster-Nachbar-Füllung über 15&nbsp;000&nbsp;km² die Julischen Alpen als glattes Hochplateau nach Süden schmiert und dem Modell ein Nährgebiet erfindet. Und: **ein DTM liefert für einen See den Wasserspiegel, nicht den Boden** – der Gardasee steht als ebene Fläche auf 65&nbsp;m im Raster, sein Boden liegt 281&nbsp;m *unter* dem Meeresspiegel. Der Bake schürft die Zungenbecken deshalb aus 22 handkuratierten Literaturtiefen aus (Bit&nbsp;4 *becken*); die Sohlen treffen die Literatur (Bodensee 145&nbsp;m ü.&nbsp;M., Garda −281, Como −213, Maggiore −179). Offen: OSM-Flussgeometrie, Kacheln als einzelne HTTP-Ressourcen + Manifest; Übergangs-UX Rheinkarte ↔ Gebiet (Brotkrume, „Gebiet öffnen").

**Die Pipeline hat zwei weitere Stufen bekommen** (Aug/Sep 2026) – statische Feldparameter statt nur einer Datei fürs Zeichnen. `bake-overlays.mjs` rechnet **elf Overlays** (Wasser, Höhe, Steigung, Relief, **Exposition**, Seegrund, Gangbarkeit, Erhaltung im Boden, Fels, Boden, Fundstelle) über alle drei Ebenen und schreibt sie nach `pipeline/out/karte.sqlite` – jeden Wert **zweimal**: als 32×32-Kachel (BLOB, Auslieferung, über den Inhalts-Hash unveränderlich cachebar) und als Zellwert mit Index (Abfrage). Das ist kein Duplikat aus Versehen: ein Rasterfeld ist sein eigener Index, Zelle → Offset ist Arithmetik – aber eine Attributfrage braucht einen echten. Stufe 4 (`ingest/ingest.mjs`) überträgt das Ganze nach Postgres. Zwei Folgen: **Exposition ist damit erledigt** (sie steht oben noch als offen, weil sie es lange war), und die Karte ist erstmals *befragbar* – „wo liegt Kieselgestein und Feuchtboden" beantwortet ein Index statt eines Scans über Millionen Zellen. Ausführlich in Kap. 4 („Befund, Bedingung, Zustand") und Kap. 8 („Ein Knopf, der die Welt einliest"). {.dim}

**Der Untergrund ist angebunden** (Sep 2026): drei der elf Overlays – `fels`, `boden`, `fundstelle` – waren angemeldet und leer, weil die Geometrie fehlte. Aus swissGEOCOVER2D (swisstopo, 23 Blätter des Geologischen Atlas 1:25 000) kommen jetzt **20 155 Festgestein-, 24 025 Lockergestein-Polygone und 70 Fundstellen**. Zwei Befunde daraus gehören ins Handbuch, weil sie über die Geologie hinausgehen: **Erstens zeigten beide Mapper auf ein Vokabular, das die Daten nicht führen** – handkuratierte Tabellen auf Katalogcodes, die swisstopo im GeoPackage gar nicht ausliefert, und im Katalog gibt es keine Brücke dorthin. Eine Übersetzungstabelle ist erst fertig, wenn sie einmal gegen echte Daten gelaufen ist; vorher ist sie eine Vermutung über die Quelle. **Zweitens steckte das Silex-Signal nicht im Hauptgestein, sondern im Beisatz** (`Kalkstein: kieselig: Bioklasten`) – eine Zuordnung nach dem Kopfterm hätte 3 260 Merkmale als Kalk geführt und genau das Material verloren, um das es vor dem Metall geht. Die Bestätigung, dass die Projektion stimmt, kam von den sechs Pfahlbauten der Quelle: sie liegen am Südufer des Bodensees, wo sie auch wirklich liegen. Ausführlich in `pipeline/README.md`. {.dim}

## Der Renderer: three.js, und eine Kamera von der Welt bis zum Feld {concept}

Entschieden im August 2026: **die Karte wird in three.js gezeichnet, nicht in DOM und nicht in 2D-Canvas.** Und die Folgeentscheidung, die eigentlich die grössere ist: **Weltkarte, Brett und Feld-Explorer sind keine drei Bildschirme, sondern drei Entfernungen derselben Kamera.** Es gibt keinen Moduswechsel, den man umlegen könnte – es gibt eine Zahl, die Bildbreite in Kilometern, und aus ihr folgt alles andere: welche Kartenebene gilt, wie ein Feld gebaut wird, ob es Marken trägt.

Der Anlass war eine Messung, keine Vorliebe. Die Pipeline liefert heute drei Ebenen mit zusammen 2 215 705 Zellen, davon **2 126 280 allein auf Ebene 2** (0.4 km/Hex). Die handgepflegte Welt in `world.ts` hat ~880. DOM trägt das nicht, und mit Terrain- und Geologiedaten wächst die Zahl weiter.

::: why Millionen sind ein Datenproblem, kein Zeichenproblem
Ein Bildschirm von 1400×900 fasst bei 52 px Hexbreite ~540 Felder; an der Ebenengrenze (~9 px) ~15 500. Der Renderer sieht damit **nie mehr als ~15 000 Felder**, egal wie gross die Karte wird – das besorgt der Ebenenwechsel. Was mit der Kartengrösse wächst, ist allein, was nachgeladen, dekodiert und wieder **ausgeworfen** werden muss. Genau diese Hälfte hat die Pipeline schon gelöst: sie liefert 32×32-Kacheln als Byte-Layer, leere Kacheln entfallen. Die Kachel der Pipeline ist deshalb auch die Baueinheit des Renderers – die Einheit, in der Daten ankommen, ist die einzige, in der man sie sinnvoll wieder freigibt.
:::

**Drei Baustufen, die wegnehmen statt zu verkleinern** – dieselbe Haltung wie die Detailstufen `S/M/L` am Feld, nur eine Ebene tiefer, beim Bauen: unter 12 px/Hex ist eine ganze Kachel **ein Rechteck mit gemalter Textur** (ein Zeichenaufruf); zwischen 12 und 46 px wird sie **Geometrie** (Deckfläche, ein Flankenband, Höhe aus dem Höhenlayer, Kontaktverschattung in die Eckenfarben gebacken); ab 46 px kommt der volle Plättchen-Bauplan dazu (Fase, zwei Flankenbänder) – **und erst dort die Marken**.

::: why Warum die Marken HTML bleiben
Krone, Wange, Ferse, Fuss und Zahl werden **nicht** in die Szene gezeichnet, sondern liegen als HTML über der Leinwand und hängen an der projizierten Feldmitte. Das ist dieselbe Arbeitsteilung wie im Diorama – three.js macht die Miniaturwelt, die Bedienung macht der Baukasten – und sie hat eine Folge, die zählt: **die Anatomie-Arbeit am Feld überlebt den Rendererwechsel.** Slots bleiben Prozentversätze auf einem Ring, Marken bleiben in `em` gegenüber der gemessenen Hexbreite, Zeichen bleiben SVG. Was sich ändert, ist allein die Quelle der Mitte: `Vector3.project()` statt `offsetLeft`. Das Marken-Budget muss dabei nicht mehr verwaltet werden – unterhalb der nahen Stufe gibt es gar keine Marken, also kann es kein Gedränge geben.
:::

Zwei Befunde, die erst 3D hervorgebracht hat, und beide sind Regeln, keine Anekdoten. **Verkürzung:** in der Schrägsicht ist die Silhouette eines Feldes kein Sechseck im Verhältnis 1:1.1547 mehr, sondern ein gedrücktes – der Slot-Ring muss um `sin(Neigung)` mit, sonst liegen Krone und Fuss in der Luft über der Deckfläche, und zwar umso weiter, je flacher der Blick. **Die Schürze war der Fehler, nicht das Gelände:** mit einer festen Unterkante unter jedem Feld stand auch auf der Talsohle eine Wand um jedes Plättchen, und das Brett las sich als Schlucht; die Schürze reicht bis knapp unter den *tiefsten Nachbarn*, mit einer Klemme gegen Felskanten, die sonst sechs Hexbreiten hoch würden.

**Die Höhe ist ein Schalter, und sie bewegt sich.** Gelände an ↔ alle Felder auf einer Ebene, über 0.6 s auf `cubic-bezier(0.16, 1, 0.3, 1)` – derselben Kurve, nach der sich die Shell bewegt. Dass die Welt derselben Regel folgt wie das Menü darüber, ist kein Detail: es ist der Unterschied zwischen einem Programm mit Animationen und einem, das sich wie ein Stück bewegt. Technisch ist es ein Regler im Vertex-Shader – jede Ecke trägt ihre zweite Höhe mit, ein Uniform mischt –, also kein Neubau und kein Ruckler; im flachen Zustand behält jedes Feld eine gleich dicke Lippe, denn wenn die Höhe nichts mehr sagt, darf sie auch nicht mehr durch die Dicke sprechen.

**Und daraus folgt die erste Linse.** Das zweite Höhenfeld heisst nicht „flach", es heisst *die andere Höhe* – der Schalter ist also ein **Überblender zwischen zwei Höhenfeldern**, und „alles flach" ist nur das langweiligste zweite Feld. Das erste interessante ist das **Wissen**: nicht eine zweite Grösse, sondern dieselbe, gezeigt nur dort, wo jemand war. **Das Relief einer Karte ist das, was vermessen wurde** – man kennt den Berg nicht, bevor man ihn begangen hat. Unbegangenes Land liegt flach und blass, gesehenes gedrückt, vergessenes fast eben und behält nur seine Umrisse. Damit hat der Nebel des Ungespielten (Kap. 2) und der Nebel des Vergessens (Kap. 8) endlich einen Körper statt einen Grauton, und das Wachstumsmodell aus `kartenwachstum-v1` ist nicht mehr eine Rechnung, sondern eine Landschaft: das Heim-Budget als Fläche, die Fern-Wege als Korridore mit blinden Flanken. Verschiebt man das Lager, **sinkt das alte Gebiet zurück** – das verlassene Dorf bleibt liegen, im Nebel.

::: why Das Wissen wird immer auf Ebene 1 gerechnet
Wissen hat *eine* Auflösung, das Gelände hat drei. Liefe die Kostensuche auf der gerade gezeichneten Ebene, änderte sich beim Zoomen, was man weiss – und das wäre Unsinn: **Zoomen ist Hinschauen, nicht Hingehen.** Jede andere Ebene fragt darum auf Ebene 1 nach. Die Nebenwirkung gehört zur Sache: auf Ebene 2 sieht man dem Nebel seine 2-km-Körnung an. Das ist richtig – man kennt ein Tal, nicht einen 400-Meter-Fleck.
:::

::: why Der flache Zustand ist ein Werkzeug, keine Vorführung
Flach ist die Geländeklassifikation **besser lesbar** als im Relief: Ufer, Hang und Flachland trennen sich sichtbar, die im Schrägblick unter ihren eigenen Flanken verschwinden. Das Relief erzählt, wo man ist; die Ebene erzählt, was es ist. Beides zusammen geht nicht – und darum ist der Schalter kein Effekt, sondern die zweite Ansicht derselben Karte, so wie der Feld-Explorer die zweite Ansicht desselben Feldes ist.
:::

Offen: ob der Ebenenwechsel beim Durchzoomen sichtbar **ploppt** (die Ferntextur ist flach, die Geometrie hat Relief); die Diorama-Palette ist für Spielplättchen gemacht, nicht für Hochgebirge – Bergland auf 1 900 m liest sich als Wiese; und wie die Ebene-2-Zellen (0.4 km) zu den Spielfeldern der Ebene 1 (2 km) stehen, wenn dort wirklich gespielt wird. {.dim}

Prototyp: `feld-labor-threejs-v1.html` (Vorgänger der Anatomie: `feld-labor-v1`, Bauplan und Palette aus `plaettchen-pfade-v1`, Kachelformat und Ebenenschwelle aus `rhein-tiles-v4`). Debug: `?sprung=welt|tal|brett|feld`, `?spann=`, `?zelle=`, `?ebene=`, `?stufe=`, `?ueber=`, `?hoehe=`, `?flach=`, `?marken=`, `?still`. Braucht einen Server – ES-Module laden unter `file://` nicht. {.dim}
